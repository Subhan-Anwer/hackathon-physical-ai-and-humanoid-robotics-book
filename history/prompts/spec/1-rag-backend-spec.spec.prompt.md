...
branch: main
user:
command: "Create a complete specification for a RAG backend API for a Docusaurus-based technical book"
labels: ["backend", "spec", "rag", "api"]
links:
  spec: null
  ticket: null
  adr: null
...
Create a complete specification for a RAG backend API for a Docusaurus-based technical book.

Context:
- The book is about Physical AI & Humanoid Robotics
- The frontend is a Docusaurus site
- The backend will be built with FastAPI (Python)
- LLM provider: Cohere
...
Write the spec to: @backend/specs/spec.md
